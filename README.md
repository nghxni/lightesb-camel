...
| `lightesb-cli.jar` | CLI 自动化、Action 目录离线派生与在线只读查询工具；Action 评分使用同目录 `.sha256` 校验 |
| `lightesb-cli.jar.sha256` | CLI JAR 完整性摘要；Action 评测缺失或不匹配时直接失败 |
