...
- 已批准 Action 会话内需要保留 digest lineage 时，使用成对的 `--action-session-id --expected-scope-digest`，只能与 `--save-remote` 组合且不返回日志。digest 从最新 session JSON 读取；STALE、冲突或恢复失败时停止，不回退普通 apply 绕过审批。
