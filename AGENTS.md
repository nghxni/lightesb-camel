...
- 用户要求规则同步并提交、推送时，先按各仓库已有约定和本次授权列出全部目标仓库与分支，再逐项核验本次文件内容、目标分支提交号与实时远端状态。当前检出分支推送成功不代表全部同步完成；最终列出每个目标分支的提交号，缺失、内容不一致或远端不可核验时明确报告未完成。不得据此扩大到未授权仓库、历史分支或包外交付材料。
